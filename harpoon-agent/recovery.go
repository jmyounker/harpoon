...
func recoverContainers(containerRoot string, r *registry, pdb *portDB, vols volumes, supervisorPath string, debug bool) {
	...
	for _, containerFilePath := range containerFilePaths {
		...
		err := recoverContainer(id, containerRoot, r, pdb, vols, supervisorPath, debug)
		if err == nil {
...
func recoverContainer(id string, containerRoot string, r *registry, pdb *portDB, vols volumes, supervisorPath string, debug bool) error {
	...
	c := newContainer(id, containerRoot, vols, agentConfig, debug, pdb, supervisorPath)
